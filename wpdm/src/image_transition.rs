...
impl ImageTransition {
    fn read_image_as_argb<P: AsRef<Path>>(
        path: P,
        width: u32,
        height: u32,
        ...
        let buff = {
            ...
            if current.width() != width || current.height() != height {
                current = current.resize(width, height, image::imageops::FilterType::Triangle);
            }
            current.to_rgba8()
        };
    ...
    pub fn get_frame(&mut self) -> Vec<u8> {
        ...
        if self.frame_count.is_multiple_of(10) || self.frame_count == next_count {
            self.is_finished = (frame == self.final_image) || next_count == self.frame_count;
        }
