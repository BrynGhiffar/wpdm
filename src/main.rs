...
use smithay_client_toolkit::{
    ...
    delegate_keyboard,
    delegate_layer,
    delegate_output, delegate_pointer,
...
use wayland_client::{
    globals::registry_queue_init, protocol::{wl_keyboard, wl_output, wl_pointer, wl_seat, wl_shm, wl_surface}, Connection, Proxy, QueueHandle
};

use crate::{image_transition::ImageTransition, layer::WallpaperLayer};
...
fn main() {
    ...
    let mut wpdm = WallpaperLayer::new(&args).unwrap();
    wpdm.run().unwrap();
}
...
fn _main() {
    ...
    let width = 1920;
    let height = 1080;
    // TODO: Need a way to detect device resolution
...
impl OutputHandler for SimpleLayer {
    ...
    fn new_output(
        ...
        if let Some(output) = self.output_state.info(&output) {
            dbg!(output.logical_size);
            dbg!(output.name);
        }
    }
...
impl SimpleLayer {
    pub fn draw(&mut self, qh: &QueueHandle<Self>) {
        ...
        let src = self.transition.get_frame();
        canvas.copy_from_slice(&src);
